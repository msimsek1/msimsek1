...
function _initPageSlider($context) {
    ...
    self.sliderCorrectVideo = function () {
        var curVideoW = parseFloat(curVideoSize.split('x')[0]);
        var curVideoH = parseFloat(curVideoSize.split('x')[1]);
        if (wndW / curVideoW > wndH / curVideoH) {
            $sliderVideoSlide.css({
                width: wndW,
                height: curVideoH * wndW / curVideoW,
                marginTop: (wndH - curVideoH * wndW / curVideoW) / 2,
                marginLeft: 0
            });
        } else {
            $sliderVideoSlide.css({
                width: curVideoW * wndH / curVideoH,
                height: wndH,
                marginTop: 0,
                marginLeft: (wndW - curVideoW * wndH / curVideoH) / 2
            });
        }
    };
    ...
    self.sliderShowNext = function () {
        ...
        if (index + 1 >= slides.length) {
            self.sliderShowSlide(0);
            return;
        }

        self.sliderShowSlide(index + 1);
    };
    self.sliderShowPrev = function () {
        ...
        if (index - 1 < 0) {
            self.sliderShowSlide(slides.length - 1);
            return;
        }
